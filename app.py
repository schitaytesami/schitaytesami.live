...
def stats_get():
	...
	return flask.Response(response = json.dumps(dict(
		...
		users = list(User.raw(
			'SELECT u1.id, u1.display, u1.num_votes, u1.num_seconds, u1.num_stations, u1.num_notes, u1.clips, u1.num_clips, u1.bookmarks, u1.notes FROM '
				'(SELECT u.id,'  
				'	u.display, ' 
				...
				#'LEFT OUTER JOIN '
				#'(SELECT u.id, IFNULL(GROUP_CONCAT((SELECT DISTINCT(n.id) FROM Event n WHERE n.creator_id = u.id AND n.type == "note")), "") as notes '
				#'FROM User u '
				#'LEFT OUTER JOIN Event n ON n.creator_id = u.id AND n.type = "note" '
				#'GROUP BY u.id '
				#') as u3 '
				#'ON u3.id = u1.id '
